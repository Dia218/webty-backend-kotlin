    ...
    fun vote(
        ...
        val response =
            voteService.vote(
                webtyUserDetails,
                voteRequest,
            ) // to do: voteService 에서 PageDto<SimilarResponse> 를 반환하도록 수정
        val responsePageDto = PageMapper.toPageDto(response)
        logger.info { "VoteService 실행 로그" }
        val destination = "/topic/vote-results/${voteRequest.similarId}"
        simpMessagingTemplate.convertAndSend(destination, responsePageDto)
